...
from bot.keyboards.promo_code import (
...
async def handle_create_promo_code_selection(callback_query: CallbackQuery):
    ...
    if promo_code_type == PromoCodeType.SUBSCRIPTION:
        ...
        await callback_query.message.answer_photo(photo=URLInputFile(photo_link, filename=photo_path),
                                                  caption=caption,
                                                  reply_markup=reply_markup)
...
async def handle_create_promo_code_subscription_selection(callback_query: CallbackQuery):
    await callback_query.answer()
