...
✨ Чтобы улучшить фотографию с <b>Gfpgan</b>, введите команду /gfpgan, а затем пришлите фотографию для улучшения
...
✨ /gfpgan - <b>Творчество с Gfpgan</b>: Улучшите качество изображения
...
🎨 <b>GFPGAN: Мастер восстановления</b>
- <i>Восстановление старых фотографий</i>: Превратите поврежденные или потерявшие качество снимки в яркие и четкие изображения.
- <i>Улучшение качества</i>: Повышайте разрешение и детализацию своих фотографий, чтобы они выглядели как новые.
- <i>Сохранение исторического наследия</i>: Реставрируйте семейные архивы и исторические документы, чтобы сохранить их для будущих поколений.
- <i>Сглаживание и устранение шумов</i>: Уберите цифровые артефакты и шумы, чтобы ваши снимки выглядели профессионально.
- <i>Персонализированные проекты</i>: Создавайте фотокниги, коллажи и другие проекты с улучшенными изображениями для особых событий и памятных моментов.
"""
    INFO_MUSIC_GEN = """
🤖 <b>Вот, что модель может сделать для вас:</b>
...
"""
    SWITCHED_TO_GFPGAN = """
✨ <b>Добро пожаловать в мир GFPGAN!</b>

Вы успешно переключились на модель <b>GFPGAN</b> — место, где старые и испорченные изображения восстанавливаются до их первоначальной славы. Хотите увидеть свои старые фотографии, ожившие с потрясающей ясностью? Просто отправьте мне фото, и позвольте <b>GFPGAN</b> сделать свою магию.

Ваши восстановленные воспоминания ждут вас! 🎉
"""
...
"""
    ...
    GFPGAN_REQUESTS = "✨ GFPGAN изображения"
    GFPGAN_REQUESTS_DESCRIPTION = "Начните восстанавливать старые изображения или неудачные фото ✨"
...
"""
    # GFPGAN
    GFPGAN_START = """
🌟 <b>Давайте вернем красоту вашим фотографиям!</b>

Готовы? Погружаемся в мир восстановления! 🚀

🌈 Отправьте мне фотографию, которую нужно улучшить, и начните своё восстановительное путешествие 👇
"""
...
━ ✨ <b>GFPGAN</b>:
    ...
    ┗ Дополнительная: {additional_usage_quota[Quota.GFPGAN]}
━ 🎵 <b>MusicGen</b>:
    ...
    def package(currency: Currency, page: int):
        ...
        if page == 0:
            ...
            description = (
                ...
                "✨ <b>GFPGAN</b>: Реставрируйте и восстанавливайте фотографии!"
            )
        elif page == 2:
            description = (
...
"""
    ...
    @staticmethod
    def gfpgan_forbidden(available_images: int):
        return f"""
 <b>Упс, небольшая проблема!</b> 🚧
...
"""
    ...
    def switched(model: Model, model_version: str):
        if model == Model.CHAT_GPT and model_version == ChatGPTVersion.V3_Turbo:
            ...
            return Russian.SWITCHED_TO_GFPGAN
    ...
    def processing_request_gfpgan():
        texts = [
            "Погружаюсь в измерение восстановления изображений... 🌌🖼️",
            "Восстанавливаю изображения, как цифровой реставратор... 🧑‍🎨🖌️",
            "Улучшение фотографий быстрее, чем вспышка камеры... 📸⚡",
            "Пробуждаю магию восстановления изображений... ✨📷",
            "Произвожу алхимию пикселей, возвращая четкость... 🧙‍🔬",
            "Запускаю машину для улучшения изображений... 🤖🛠️",
            "Готовлю эликсир для восстановления фотографий... 🧪📸",
            "Творю чудеса в мире реставрации изображений... 🧚‍🖼️",
            "Организую симфонию пикселей... 🎼🖥️",
            "Леплю новые детали в моей цифровой арт-студии... 🎨👩‍🎨",
            "Варю котел волшебства восстановления изображений... 🧙‍🔮",
            "Строю изображения, как великий архитектор... 🏗️👷‍",
            "Начинаю мистический поиск идеального восстановления... 🗺️🔍",
            "Запускаю ракету улучшений фотографий... 🚀📷",
            "Отправляюсь в галактическое путешествие восстановления изображений... 🌌🌟"
        ]

        text = random.choice(texts)
        text += "\n\n⚠️ Реставрация может занять до 2 минут"
