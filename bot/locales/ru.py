...
Основное:
- Ты бот в Telegram
- Создай высокоэффективное, небольшое, но информативное сообщение о возможностях AI модели: ChatGPT 4.0 Omni Mini ✉️
- Используй простые слова, избегай сложных терминов, чтобы даже пользователь без опыта и знаний в нейросетях всё понял
- Говори от второго лица

Цели:
- Объяснить, какие задачи модель может выполнять.
- Указать, какие боли/проблемы она решает.
- Представить информацию структурированно, красиво и с использованием маркеров, списков или разделов.
- Добавь мотивацию к действию, чтобы пользователь захотел воспользоваться моделью, но не говори, что это мотивация к действию.

Структура сообщения:
- Заголовок с эмодзи, привлекающий внимание. Но слишком много эмодзи не нужно.
- Краткое описание модели: что это такое, с чем её едят и в чем её уникальность.

Основные возможности:
- Список задач, которые решает модель, с коротким описанием каждого пункта.
- Пример использования или сценарии, где модель особенно полезна.

Как она решает боли пользователя:
- Какие типичные проблемы закрывает модель.
- Почему она эффективна именно для этих задач.

Призыв к действию:
- Пользователь сразу может написать запрос в чат, предложи сразу несколько очень простых примеров
- Если он хочет поменять модель он может использовать команду /model
- Если он хочет поменять настройки модели он может использовать команду /settings

Форматирование:
- Используй жирный текст для выделения ключевых возможностей.
- Раздели задачи по категориям, если их много.
- Добавь вдохновляющий завершающий текст.
"""
    QUICK_GUIDE = """
...
"""
    INFO_TEXT_MODELS = """
🤖 <b>Выберите текстовую модель, про которую вы хотите получить информацию:</b>
"""
    INFO_IMAGE_MODELS = """
🤖 <b>Выберите графическую модель, про которую вы хотите получить информацию:</b>
"""
    INFO_MUSIC_MODELS = """
🤖 <b>Выберите музыкальную модель, про которую вы хотите получить информацию:</b>
"""
    INFO_VIDEO_MODELS = """
🤖 <b>Выберите видео модель, про которую вы хотите получить информацию:</b>
"""
    INFO_CHATGPT = """
...
Привет! Я всегда стремлюсь к улучшению, и ваша обратная связь для меня как золото ✨
...
И помните, каждое ваше замечание — это шаг к тому, чтобы я стал ещё круче. С нетерпением жду ваши мысли! 💌
"""
    FEEDBACK_SUCCESS = """
...
"""
    PLAY_BOWLING_GAME = "🎳 Сыграть в боулинг"
...
"""
    PLAY_SOCCER_GAME = "⚽️ Сыграть в футбол"
...
"""
    PLAY_BASKETBALL_GAME = "🏀 Сыграть в баскетбол"
...
"""
    PLAY_DARTS_GAME = "🎯 Сыграть в дартс"
...
"""
    PLAY_CASINO_GAME = "🎰 Сыграть в казино"
...
Возвращайтесь через <i>{hours_before_limit_update} ч. {minutes_before_limit_update} мин.</i> и покажите, на что вы способны! 👏
"""
...
"""
    CHOOSE_CLAUDE_MODEL = """
Для выбора <b>Claude 📄</b> модели нажмите на кнопку ниже 👇
"""
    CHOOSE_GEMINI_MODEL = """
Для выбора <b>Gemini ✨</b> модели нажмите на кнопку ниже 👇
"""
    SWITCHED_TO_AI_SETTINGS = "⚙️ Настройки модели"
    SWITCHED_TO_AI_INFO = "ℹ️ Подробнее про модель"
    SWITCHED_TO_AI_EXAMPLES = "💡 Показать примеры"
...
"""
    ...
    CHANGE_AI_MODEL = "🤖 Поменять AI модель"
    ...
    VIDEO_SUCCESS = "✨ Вот ваше созданное видео 🎞"
    FILE_TOO_BIG_ERROR = """
🚧 <b>Ой-ой!</b>
...
"""
    ...
    CLAUDE_3_SONNET_EXAMPLE = "👇 Вот так ответил бы на ваш запрос *Claude 3.5 Sonnet* 💫"
    GEMINI_1_PRO_EXAMPLE = "👇 Вот так ответил бы на ваш запрос *Gemini 1.5 Pro* 💼"
    MIDJOURNEY_EXAMPLE = "☝️ Вот такие изображения на ваш запрос нарисовал бы <b>Midjourney 🎨</b>"
...
"""
    ...
    SECONDS_60 = "🔹 60 секунд (1 минута)"
    SECONDS_180 = "🔹 180 секунд (3 минуты)"
    SECONDS_300 = "🔹 300 секунд (5 минут)"
    SECONDS_600 = "🔹 600 секунд (10 минут)"
...
🎁 Хотите вернуть предыдущие роли? Нажмите на кнопку снизу, чтобы оформить подписку или купить новый пакет:
"""
...
Чтобы разблокировать возможность вести несколько чатов нажмите на одну из кнопок снизу:
"""
...
"""
    ...
    ┗ 🗯 Тип отправки: {Russian.DOCUMENT if model_info.get(UserSettings.SEND_TYPE, SendType.IMAGE) == SendType.DOCUMENT else Russian.IMAGE}"""
            ...
            facts = f"""⚙️ Настройки:
    ┗ 🗯 Тип отправки: {Russian.VIDEO if model_info.get(UserSettings.SEND_TYPE, SendType.AUDIO) == SendType.VIDEO else Russian.AUDIO}"""
        elif model_type == ModelType.VIDEO:
            facts = f"""⚙️ Факты и настройки:
    ┣ 📷 Работа с фото: {'Да ✅' if model_info.get('support_photos', False) else 'Нет ❌'}
    ┣ 📐 Соотношение сторон: {'Пользовательское' if model_info.get(UserSettings.ASPECT_RATIO, AspectRatio.CUSTOM) == AspectRatio.CUSTOM else model_info.get(UserSettings.ASPECT_RATIO)}
    ┣ 📏 Длительность: {model_info.get(UserSettings.DURATION, 5)} секунд
    ┗ 🗯 Тип отправки: {Russian.DOCUMENT if model_info.get(UserSettings.SEND_TYPE, SendType.VIDEO) == SendType.DOCUMENT else Russian.VIDEO}"""
        else:
