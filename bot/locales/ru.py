...
━ <b>Продвинутые</b>:
    ...
    ┣ 🧩 ChatGPT o1-mini{f': доп. {additional_usage_quota[Quota.CHAT_GPT_O_1_PREVIEW]}' if additional_usage_quota[Quota.CHAT_GPT_O_1_PREVIEW] > 0 else ''}
...
━ <b>Флагманские</b>:
    ┣ Дневной лимит: {format_number(daily_limits[Quota.CHAT_GPT_O_1_PREVIEW])}/{format_number(subscription_limits[Quota.CHAT_GPT_O_1_PREVIEW])}
    ┣ 🧪 ChatGPT o1-preview{f': доп. {additional_usage_quota[Quota.CHAT_GPT_O_1_PREVIEW]}' if additional_usage_quota[Quota.CHAT_GPT_O_1_PREVIEW] > 0 else ''}
...
"""
    ...
    ┣ 📷 Работа с изображениями: {'Да ✅' if model_info.get('support_photos', False) else 'Нет ❌'}
    ┣ 📄 Работа с документами: {'Скоро 🔜' if model_info.get('support_documents', False) else 'Нет ❌'}
    ┣ 🎙 Голосовые ответы: {'Включены ✅' if model_info.get(UserSettings.TURN_ON_VOICE_MESSAGES, False) else 'Выключены ❌'}
